...
export default function Galaxy({ stars = [], fetchPlanetsForStar, isFullscreen = false, onRequestExit }) {
  ...
  useEffect(() => {
    ...
    // Additional light for focused star
    ...
    const focusGroup = new THREE.Group(); // Group for the focused star system
    scene.add(starGroup);
    scene.add(planetGroup);
    scene.add(focusGroup);

    // Store original stars data
    const starsData = [];
    ...
    const clearFocusGroup = () => {
      while (focusGroup.children.length > 0) {
        const obj = focusGroup.children[0];
        focusGroup.remove(obj);
        if (obj.geometry) obj.geometry.dispose();
        if (obj.material) obj.material.dispose();
      }
    };
    ...
    const focusOnStar = async (clickedStar) => {
      ...
      // Hide the main star group and background
      ...
      // Remove fog for better visibility when focused
      scene.fog = null;
      
      // Create a copy of the clicked star for the focused view
      const starData = clickedStar.userData.starData;
      const originalRadius = clickedStar.userData.originalRadius;
      const scaleFactor = 8;
      
      // Create enlarged star at origin
      ...
      // Position light at the focused star
      focusLight.position.set(0, 100, 100);
      focusLight.visible = true;
      
      // Position camera to view the focused star
        ...
        try {
          ...
          if (planets && planets.length > 0) {
            planets.forEach((p, index) => {
              // Calculate planet position around the focused star
    ...
    const onClick = async (event) => {
      ...
      if (focusedStarRef.current) {
        // Click anywhere to exit focused mode
        resetView();
        return;
      }
    ...
    const animate = () => {
      ...
      // Rotate focused star
      if (focusGroup.children.length > 0) {
        focusGroup.children[0].rotation.y += 0.002;
      }
